...
window.isWritingLock = false; window.currentSampleRate = 44100; window.currentSinFreq = 1830; 
window.filteredDataLog = []; window.bufferIndex = 0; window.nextPlayTime = 0; window.isSpeakerOn = false;
window.isSimulating = false; window.currentVolume = 0.3; window.simPhase = 0; window.simPhase2 = 0;             
window.FFT_SIZE = 4096; window.renderFrameCounter = 0; window.analysisBuffer = new Float32Array(window.FFT_SIZE);
window.currentFilterMode = 'RAW'; window.f1 = 1000; window.f2 = 3000;
window.b0 = 1; window.b1 = 0; window.b2 = 0; window.a1 = 0; window.a2 = 0;

// 🔒 🚀 【第一段】八階 8-pole 穩健巴特沃斯級聯全域移位暫存器陣列最剛性絕對死鎖！
...
window.updateFilterCoefficients = function() {
    let fs = window.currentSampleRate; let fr1 = fs / window.f1; if (fr1 < 2.01) fr1 = 2.01; let o = Math.tan(Math.PI / fr1);
    if (window.currentFilterMode === 'LP') { 
        ...
        let fr2 = fs / f2Correct; if (fr2 < 2.01) fr2 = 2.01; let o2 = Math.tan(Math.PI / fr2);
        let W = o2 - o; if (W < 0.001) W = 0.001; let C = o * o2; let cBP = 1.0 + W + C;
        window.b0 = W / cBP; window.b1 = 0.0; window.b2 = -window.b0; window.a1 = 2.0 * (C - 1.0) / cBP; window.a2 = (1.0 - W + C) / cBP;
    } else { window.b0 = 1; window.b1 = window.b2 = window.a1 = window.a2 = 0; }
    if (isNaN(window.b0) || !isFinite(window.b0) || isNaN(window.a1) || !isFinite(window.a1)) { window.b0 = 1; window.b1 = window.b2 = window.a1 = window.a2 = 0; }
};

// 🔒 🚀 【🚨 第二段核心禁區絕對大死鎖】中括號下標數字數字 , ,  100% 原始完璧、電路大通電！
window.applyFilter = function(x) { 
    ...
    window.yv[2] = (window.b0 * window.xv[2]) + (window.b1 * window.xv[1]) + (window.b2 * window.xv[0]) - (window.a1 * window.yv[1]) - (window.a2 * window.yv[0]);
    ...
    window.yv2[2] = (window.b0 * window.xv2[2]) + (window.b1 * window.xv2[1]) + (window.b2 * window.xv2[0]) - (window.a1 * window.yv2[1]) - (window.a2 * window.yv2[0]);
    ...
    window.yv3[2] = (window.b0 * window.xv3[2]) + (window.b1 * window.xv3[1]) + (window.b2 * window.xv3[0]) - (window.a1 * window.yv3[1]) - (window.a2 * window.yv3[0]);
    if (isNaN(window.yv3[2]) || !isFinite(window.yv3[2])) window.yv3[2] = 0;
    // 🛑 Stage 4（總計 4 級串聯 = 8階穩健巴特沃斯，極限衰減剛性爆發至 -48 dB/Octave！）
    ...
    window.yv4[2] = (window.b0 * window.xv4[2]) + (window.b1 * window.xv4[1]) + (window.b2 * window.xv4[0]) - (window.a1 * window.yv4[1]) - (window.a2 * window.yv4[0]);
...
window.oscNode = null; window.oscNode2 = null; window.scriptNode = null; window.audioCtx = null; window.gainNode = null;
window.initAudioGlobal = function() {
    ...
    if (!window.audioCtx) { window.audioCtx = new (window.AudioContext || window.webkitAudioContext)(); window.gainNode = window.audioCtx.createGain(); window.gainNode.connect(window.audioCtx.destination); }
    window.gainNode.gain.setValueAtTime(window.isSpeakerOn ? window.currentVolume : 0.0, window.audioCtx.currentTime); if (window.audioCtx.state === 'suspended') window.audioCtx.resume();
    if (window.isSimulating) {
        window.updateFilterCoefficients(); window.oscNode = window.audioCtx.createOscillator(); window.oscNode2 = window.audioCtx.createOscillator();
        window.oscNode.frequency.setValueAtTime(window.currentSinFreq, window.audioCtx.currentTime); window.oscNode2.frequency.setValueAtTime(window.currentSinFreq * 0.4, window.audioCtx.currentTime);
        window.scriptNode = window.audioCtx.createScriptProcessor(4096, 1, 1);
        window.scriptNode.onaudioprocess = function(e) {
            let out = e.outputBuffer.getChannelData(0);
            for (let s = 0; s < e.inputBuffer.length; s++) {
                let step1 = 2.0 * Math.PI * (window.currentSinFreq / window.currentSampleRate); let step2 = 2.0 * Math.PI * ((window.currentSinFreq * 0.4) / window.currentSampleRate);
                let raw = (Math.sin(window.simPhase) + Math.sin(window.simPhase2)) * 0.5;
                window.simPhase = (window.simPhase + step1) % (2 * Math.PI); window.simPhase2 = (window.simPhase2 + step2) % (2 * Math.PI);
                let fVal = window.applyFilter ? window.applyFilter(raw) : raw; out[s] = fVal;
...
window.consumeRawBuffer = function(view) {
    for (let i = 0; i < view.byteLength; i++) {
        let fVal = window.applyFilter((view.getUint8(i) / 127.5) - 1.0); window.filteredDataLog.push(fVal);
...
// ==========================================
// 📥 【下半部：第 4, 5 段 — 真對數分貝渲染 ＋ !important 水泥灰外框事件大合龍】
// ==========================================
...
window.globalRenderLoop = function() {
    ...
    let hzPerBin = window.currentSampleRate / window.FFT_SIZE; let maxDisplayFreq = window.currentSinFreq * 1.5;
    ...
    for (let n = 0; n < magnitudes.length; n++) { 
        ...
        let dbValue = 20.0 * Math.log10(ratio); if (dbValue < -50.0) dbValue = -50.0;
...
window.renderFilterButtonLights = function() {
    ...
    Object.keys(btnIds).forEach(mode => {
        let btnEl = document.getElementById(btnIds[mode]); if (!btnEl) return;
...
document.addEventListener('click', (e) => {
    if (!e.target || !e.target.id) return; let clickId = e.target.id;
