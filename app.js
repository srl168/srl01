...
async function sendHardwareParameters() {
    ...
    view.setUint32(0, sr, true); view.setUint32(4, sf, true);
...
function renderLoop() {
    ...
    for (let i = 0; i < FFT_SIZE; i++) { 
        ...
        fftInput.push(analysisBuffer[idx] * (0.5 * (1 - Math.cos((2 * Math.PI * i) / (FFT_SIZE - 1))))); 
    }
    ...
    try {
        ...
        for (let i = 0; i < FFT_SIZE / 2; i++) { 
            let c = fftResult[i]; magnitudes[i] = Math.sqrt(c.re * c.re + c.im * c.im) / (FFT_SIZE / 2); 
            if (i > 1 && magnitudes[i] > maxMag) { maxMag = magnitudes[i]; maxIdx = i; } 
        }
        let peakFreq = maxIdx * (currentSampleRate / FFT_SIZE); 
        document.getElementById('freqVal').innerText = maxMag > 0.04 ? peakFreq.toFixed(1) + " Hz" : "0.0 Hz";
