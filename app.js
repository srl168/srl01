...
window.globalRenderLoop = function() {
    ...
    // 💡 🛠️ 恢復真・儀表本色：不論模擬測試是開是關，只要快取池內有藍牙或模擬訊號，100% 絕對放行繪製！
    ...
    // 💡 🛠️ 徹底洗淨 FFT 快取陣列：杜絕殘留數據引發的原地空轉狂抖現象！
    ...
    // ==========================================
    // 💡 🛠️ 恢復橫向 5000Hz 固定觀測全幅視野死鎖（4kHz 永遠死鎖在第 640 像素，不再因 F1 變更而消失！）
    // ==========================================
    ...
    // 💡 🛠️ 迎回標準工業分貝水平格線標尺（0 到 -50 dB 均勻等分）
    ...
    for (let n = 0; n < magnitudes.length; n++) { 
        let currentPointRealHz = n * hzPerBinHW; if (currentPointRealHz > 5000) break; // 超過 5kHz 視野上限則截斷
        let curX = (currentPointRealHz / 5000) * 800; // 🚀 100% 幾何綁定 5kHz，4kHz 註定精準鎖定在 640 像素，不再飛走！
        
        // 💡 純圖學幾何對齊：RAW 直通狀態下未衰減最大主峰 100% 精準頂格貼齊在最上方 0 dB 線上！
...
document.getElementById('simBtn')?.addEventListener('click', () => {
    ...
    document.getElementById('status').innerText = window.isSimulating ? "▶️ 離線沙盒：真・示波器圖量網絡點火！" : "狀態：模擬測試已停止，等待實體藍牙信號灌注。";
});
...
document.addEventListener('input', (e) => {
    if (e.target && e.target.type === 'range') {
        ...
        if (sliderId === "f2Slider") { 
            window.f2 = parseInt(curVal); if (nextSpan) { let dQ = 0.1 + (window.f2 / 5000.0) * 9.9; nextSpan.innerText = "Q: " + dQ.toFixed(2); }
            window.updateFilterCoefficients(); 
        }
        if (sliderId === "volumeSlider") { 
            window.currentVolume = curVal; if (nextSpan) nextSpan.innerText = Math.round(curVal * 100) + "%"; 
            if (window.gainNode && window.audioCtx && window.isSpeakerOn) { 
                window.gainNode.gain.setValueAtTime(window.currentVolume, window.audioCtx.currentTime); 
            } 
        }
    }
});
